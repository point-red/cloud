        ...
        <p-block-inner :is-loading="isLoading">
          <point-table>
            ...
            <tr
              ...
              <td class="text-right">{{ salesOrderItem.price | numberFormat }}</td>
              <td class="text-right">{{ (salesOrderItem.quantity * salesOrderItem.price) | numberFormat }}</td>
              <td class="text-center">
                <div v-if="salesOrder.form.approval_status == 0" class="badge badge-primary">{{ $t('pending') | uppercase }}</div>
                <div v-if="salesOrder.form.approval_status == -1" class="badge badge-danger">{{ $t('rejected') | uppercase }}</div>
                <div v-if="salesOrder.form.approval_status == 1" class="badge badge-success">{{ $t('approved') | uppercase }}</div>
              </td>
              <td class="text-center">
                <div v-if="salesOrder.form.cancellation_status == 1" class="badge badge-danger">{{ $t('canceled') | uppercase }}</div>
                <div v-else-if="salesOrder.form.done == 0" class="badge badge-primary">{{ $t('pending') | uppercase }}</div>
                <div v-else-if="salesOrder.form.done == 1" class="badge badge-success">{{ $t('done') | uppercase }}</div>
              </td>
              <td>
                <!-- <p-form-check-box
                  :is-form="false"
