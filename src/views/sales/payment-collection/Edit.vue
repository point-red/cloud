<template>
  <div>
    <breadcrumb>
      <breadcrumb-sales />
      <router-link
        :to="{ name: 'sales.payment-collection.index' }"
...
export default {
  ...
  methods: {
    ...mapActions('salesPaymentCollection', ['find', 'update', 'addHistories', 'getReferences']),
    ...
    salesPaymentCollectionRequest () {
      ...
      this.find({
        ...
        response.data.details.forEach(element => {
          var id = element.referenceable_id
          var obj = {
            number: element.referenceable_form_number,
            id: element.referenceable_id,
            notes: element.referenceable_form_notes,
            date: element.referenceable_form_date,
            amount: element.available,
            collected: 0,
            toCollect: element.amount
          }
          console.log(JSON.stringify(element))
          if (element.chart_of_account_id) {
            var objOther = {
              chart_of_account_id: element.chart_of_account_id,
              chart_of_account_name: element.chart_of_account.label,
              amount: element.amount,
              allocation_id: element.allocation_id,
              allocation_name: element.allocation ? element.allocation.label : null,
              notes: element.notes,
              is_debit: element.chart_of_account.type.is_debit
            }
            this.form.others.push(objOther)
          } else if (element.referenceable_type === 'SalesInvoice') {
            this.form.references.salesInvoice.push(obj)
            this.checkedRow[0].push({ id })
            this.total_invoice += parseFloat(element.amount)
          } else if (element.referenceable_type === 'SalesDownPayment') {
            this.form.references.salesDownPayment.push(obj)
            this.checkedRow[1].push({ id })
            this.total_down_payment += parseFloat(element.amount)
          } else if (element.referenceable_type === 'SalesReturn') {
            this.form.references.salesReturn.push(obj)
            this.checkedRow[2].push({ id })
    ...
    openPreview () {
      ...
      this.$router.push({ name: 'sales.payment-collection.preview', params: { source: 'edit', id: this.id } })
    },
