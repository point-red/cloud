    ...
    search () {
      ...
      if (this.type) {
        ...
        this.get({
          ...
          this.mutableLabel = null
          response.data.map((key, value) => {
            if (key.type.name.toUpperCase() === 'NET INCOME' ||
              key.type.name.toUpperCase() === 'SALES INCOME' ||
              key.type.name.toUpperCase() === 'OTHER INCOME' ||
              key.type.name.toUpperCase() === 'COST OF SALES' ||
              key.type.name.toUpperCase() === 'DIRECT EXPENSE' ||
              key.type.name.toUpperCase() === 'OTHER EXPENSE') {
              this.options.push({
                id: key.id,
                alias: key.alias,
                label: key.label,
