<template>
  <sweet-modal
    ref="print-preview"
    :title="$t('print delivery note') | uppercase"
    overlay-theme="dark"
    @close="onClose()"
  >
    <table
      id="print-delivery-note"
      class="print-delivery-note-container m-2 mb-4 mx-auto"
    >
      <thead>
        <tr>
          <td>
        ...
        <tr>
          <td>
            ...
            >
              ...
              <tr
                v-for="(row, index) in deliveryNote.items"
                :key="index"
              >
                <td>
                  {{ row.item.label }}
            ...
            >
              ...
              <div class="text-center">
                <h3>Created By</h3>
                <br><br><br>
                {{ deliveryNote.form.created_by.full_name }}
              </div>
            </div>
          </td>
        </tr>
...
export default {
  ...
  methods: {
    ...mapActions('salesDeliveryNoteHistory', ['add']),
    clear () {
      this.mutableId = null
    ...
    createHistoryPrint () {
      this.add({ id: this.deliveryNote.id, activity: 'Printed' })
        .catch(error => {
...
table.header-detail tr td:nth-child(2) {
  padding: 0 10px;
}
@media print {
  ...
  #print-delivery-note thead tr td {
    padding-top: 20px;
  }
}
</style>
