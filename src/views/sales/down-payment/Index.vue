    ...
    <div class="row">
      <p-block>
        <div class="input-group block">
          <router-link
            v-if="$permission.has('create sales down payment')"
            to="/sales/down-payment/create"
        ...
        <p-block-inner :is-loading="isLoading">
          <point-table>
            ...
            <template v-for="(salesDownPayment, index) in downPayments">
              <tr
                :key="'downpayment-' + index"
                slot="p-body"
              >
                <th>
                  <router-link :to="{ name: 'sales.down-payment.show', params: { id: salesDownPayment.id }}">
                    {{ salesDownPayment.form.number }}
                  </router-link>
                </th>
                <td>{{ salesDownPayment.form.date | dateFormat('DD MMMM YYYY HH:mm') }}</td>
                <td>
                  <template v-if="salesDownPayment.customer">
                    {{ salesDownPayment.customer.name }}
                  </template>
                </td>
                <td>{{ salesDownPayment.form.notes }}</td>
                <td class="text-right">
                  {{ salesDownPayment.amount | numberFormat }}
                </td>
                <td class="text-right">
                  {{ salesDownPayment.remaining | numberFormat }}
                </td>
                <td class="text-center">
                  <div
                    v-if="salesDownPayment.form.approval_status == 0"
                    class="badge badge-primary"
                  >
                    {{ $t('pending') | uppercase }}
                  </div>
                  <div
                    v-if="salesDownPayment.form.approval_status == -1"
                ...
                <td class="text-center">
                  <div
                    v-if="salesDownPayment.form.cancellation_status == 1"
                    class="badge badge-danger"
                  >
                    {{ $t('canceled') | uppercase }}
                  </div>
                  <div
                    v-else-if="salesDownPayment.form.done == 0"
                    class="badge badge-primary"
                  >
                    {{ $t('pending') | uppercase }}
                  </div>
                  <div
                    v-else-if="salesDownPayment.form.done == 1"
                    class="badge badge-success"
                  >
                    {{ $t('done') | uppercase }}
                  </div>
                ...
                <!-- <p-form-check-box
                  ...
                  id="check-box"
                  name="check-box"
                  @click.native="toggleCheckRow(salesDownPayment.id)"
                  :checked="isRowChecked(salesDownPayment.id)"
                  class="text-center"/> -->
                </td>
              </tr>
            </template>
...
export default {
  ...
  methods: {
    ...mapActions('salesDownPayment', ['get']),
    toggleCheckRow (id) {
