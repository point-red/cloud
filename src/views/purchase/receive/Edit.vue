<template>
  <div>
    <breadcrumb>
      ...
      <router-link :to="{ name: 'purchase.order.show', params: { id: id }}" class="breadcrumb-item">{{ purchaseReceive.form.number }}</router-link>
  ...
  created () {
    ...
    this.edit({
      ...
      if (!this.$formRules.allowedToUpdate(response.data.form)) {
        this.$router.replace('/purchase/receive/' + response.data.id)
      }
