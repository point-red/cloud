<template>
  <div>
    <breadcrumb>
      <breadcrumb-purchase/>
      <span class="breadcrumb-item active">{{ $t('purchase invoice') | uppercase }}</span>
    </breadcrumb>
      ...
      <p-block>
        <div class="input-group block">
          <router-link
            ...
            v-if="$permission.has('create purchase invoice')"
        ...
        <div class="mt-10">
          <!-- <label class="css-control css-control-primary css-checkbox mr-10">
            <input
              ...
              :checked="isRowsChecked(purchaseInvoices, checkedRow)">
        ...
        <p-block-inner :is-loading="isLoading">
          <point-table>
            ...
            <tr
              ...
              <td class="text-right">{{ purchaseInvoiceItem.quantity | numberFormat }} {{ purchaseInvoiceItem.unit }}</td>
              ...
              <td class="text-center">
                <div v-if="purchaseInvoice.form.approval_status == 0" class="badge badge-primary">{{ $t('pending') | uppercase }}</div>
                <div v-if="purchaseInvoice.form.approval_status == -1" class="badge badge-danger">{{ $t('rejected') | uppercase }}</div>
                <div v-if="purchaseInvoice.form.approval_status == 1" class="badge badge-success">{{ $t('approved') | uppercase }}</div>
              </td>
              <td class="text-center">
                <div v-if="purchaseInvoice.form.cancellation_status == 1" class="badge badge-danger">{{ $t('canceled') | uppercase }}</div>
                <div v-else-if="purchaseInvoice.form.done == 0" class="badge badge-primary">{{ $t('pending') | uppercase }}</div>
                <div v-else-if="purchaseInvoice.form.done == 1" class="badge badge-success">{{ $t('done') | uppercase }}</div>
              </td>
              <td>
                <!-- <p-form-check-box
                  :is-form="false"
                  id="check-box"
                  name="check-box"
                  @click.native="toggleCheckRow(purchaseInvoice.id)"
                  :checked="isRowChecked(purchaseInvoice.id)"
                  class="text-center"/> -->
              </td>
            </tr>
            </template>
...
export default {
  ...
  methods: {
    ...
    toggleCheckRows () {
      if (!this.isRowsChecked(this.purchaseInvoices, this.checkedRow)) {
    ...
    bulkApprove () {
      this.bulkApprove({
        purchaseInvoices: this.checkedRow
      }).then(response => {
        this.checkedRow = []
        this.getPurchaseInvoice()
      })
