        ...
        >
          <point-table>
            ...
            <template v-for="(purchaseOrder, index) in purchaseOrders">
              <template v-for="(purchaseOrderDetail, index2) in purchaseOrder.details">
                <tr
                  :key="'purchase-order-' + index + '-' + index2"
                  slot="p-body"
                >
                  ...
                  <td class="text-center">
                    <router-link :to="{ name: 'finance.cash-advance.show', params: { id: purchaseOrder.id }}">
                  ...
                  <td class="text-center">
                    {{ purchaseOrderDetail.account.number }} - {{ purchaseOrderDetail.account.alias }}
                  </td>
                  <td class="text-center">
...
export default {
  ...
  methods: {
    ...
    async generateReport () {
      this.isLoading = true
      try {
        const { data: { data: purchaseOrders } } = await axios.get('/finance/cash-advances', {
          params: {
            join: 'form,details,account,employee',
            sort_by: '-form.number',
            group_by: 'purchase_order.id',
