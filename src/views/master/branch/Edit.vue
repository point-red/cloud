<template>
  <div>
    <breadcrumb>
      ...
      <router-link :to="'/master/branch/'+form.id" class="breadcrumb-item">{{ branch.name | uppercase }}</router-link>
...
export default {
  ...
  methods: {
    ...mapActions('masterBranch', ['find', 'update']),
    onSubmit () {
      this.update(this.form)
        .then(response => {
          ...
          this.$router.push('/master/branch/' + this.id)
        }).catch(error => {
          this.isSaving = false
