    ...
    <form class="row" @submit.prevent="onSubmit">
      <p-block :title="$t('employee assessment')" :header="true">
        <p-block-inner :is-loading="isLoading">
          <p-form-row id="name" :label="$t('name')">
            <div slot="body" class="col-lg-9 col-form-label">{{ employee.name }}</div>
          </p-form-row>
          <p-form-row id="assessment-date" :label="$t('assessment period')">
          ...
          <p-form-row id="assessment-category" :label="$t('assessment category')">
            <div v-if="form.template.name" slot="body" class="col-lg-9 col-form-label">
              <a
                href="javascript:void(0)"
                @click="$refs.assignKpiTemplate.show(id)"
              >{{ form.template.name }}</a>
            </div>
            <div v-else slot="body" class="col-lg-9" @click="$refs.assignKpiTemplate.show(id)">
          ...
          <p-table>
            <tr slot="p-head">
              ...
              <th class="font-size-h6 font-w700 text-center">{{ $t('notes') | uppercase }}</th>
              <th class="font-size-h6 font-w700 text-center">{{ $t('attachment') | uppercase }}</th>
              <th class="font-size-h6 font-w700 text-center">{{ $t('score') | uppercase }}</th>
              <th
                class="font-size-h6 font-w700 text-center"
              >{{ $t('score percentage') | uppercase }}</th>
              <th class="font-size-h6 font-w700 text-center">{{ $t('description') | uppercase }}</th>
              <th />
            </tr>
            <template v-for="group in form.template.groups" slot="p-body">
              ...
              <tr v-for="(indicator, index) in group.indicators" :key="'indicator' + indicator.id">
                ...
                <td class="text-center">
                  <a
                    v-show="!indicator.selected || indicator.selected.notes_indicator === '' || indicator.selected.notes_indicator === undefined"
                    href="javascript:void(0)"
                    class="btn btn-sm btn-primary"
                    @click="$refs.notes.show(indicator)"
                  >
                    <i class="si si-note" />
                  </a>
                  <a
                    ...
                    v-if="indicator.selected && indicator.selected.notes_indicator !== ''"
                  >{{ indicator.selected.notes_indicator }}</a>
                </td>
                <td class="text-center">
                  <label
                    :for="'file-' + indicator.id"
                    class="btn btn-sm btn-primary mt-1"
                    @click="setIndicatorIdAtt(indicator.id)"
                  >
                    <i class="si si-paper-clip" />
                  </label>
                  <input
                  ...
                  <span
                    class="ml-2"
                    v-if="indicator.selected && indicator.selected.attachment"
                  >{{ indicator.selected.attachment.name }}</span>
                </td>
                <td class="text-center">
                  <a
                  ...
                  <span v-if="indicator.selected">{{ indicator.selected.score | numberFormat }}</span>
                  <span
                    v-else-if="indicator.automated_code && indicator.score"
                  >{{ indicator.score | numberFormat }}</span>
                  <span v-else-if="indicator.automated_code && !indicator.score">{{ 0 }}</span>
                </td>
                <td class="text-center">
                  <span
                    v-if="indicator.selected"
                  >{{ indicator.selected.score_percentage | numberFormat }}</span>
                  <span
                    v-else-if="indicator.automated_code && indicator.score_percentage"
                  >{{ indicator.score_percentage | numberFormat }}</span>
                  <span v-else-if="indicator.automated_code && !indicator.score">{{ 0 }}</span>
                </td>
                <td class="text-center">
                  <span v-if="indicator.selected">{{ indicator.selected.description }}</span>
                </td>
                <td class="text-center">
                  <span>
                    <button
          ...
          <p-form-row :label="$t('comment')">
            <div slot="body" class="col-lg-9 col-form-label">
              <textarea v-model="form.template.comment" class="form-control" rows="3" />
            </div>
          </p-form-row>
...
export default {
  ...
  methods: {
    ...
    addedNotes ({ indicatorId, notes }) {
      ...
      if (this.form.template.groups[groupIndex].indicators[indicatorIndex].selected !== undefined) {
        this.$set(this.form.template.groups[groupIndex].indicators[indicatorIndex].selected, 'notes_indicator', notes)
      } else {
        this.$set(this.form.template.groups[groupIndex].indicators[indicatorIndex], 'selected', { notes_indicator: notes })
      }
    ...
    onFileChange (e) {
      ...
      const indicatorIndex = this.form.template.groups[groupIndex].indicators.findIndex(o => o.id === indicatorId)
      if (this.form.template.groups[groupIndex].indicators[indicatorIndex].selected !== undefined) {
        this.$set(this.form.template.groups[groupIndex].indicators[indicatorIndex].selected, 'attachment', file)
      } else {
        this.$set(this.form.template.groups[groupIndex].indicators[indicatorIndex], 'selected', { attachment: file })
      }
      console.log(this.form)
    ...
    addedScore ({ indicatorId, score, notes }) {
      ...
      // update data
      if (this.form.template.groups[groupIndex].indicators[indicatorIndex].selected !== undefined) {
        $selected = this.form.template.groups[groupIndex].indicators[indicatorIndex].selected;
        $selected = { ...selected, ...score }
        this.$set(this.form.template.groups[groupIndex].indicators[indicatorIndex], 'selected', selected)
      } else {
