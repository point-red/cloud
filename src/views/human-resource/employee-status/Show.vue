          ...
          <div class="row">
            ...
            <div class="col-sm-12 mb-20">
              ...
              <router-link
                ...
                v-if="$permission.has('update employee status')"
              ...
              <button
                type="button"
                @click="onDelete()"
                v-if="$permission.has('delete employee status')"
...
export default {
  ...
  computed: {
    ...mapGetters('EmployeeStatus', ['status', 'statuses']),
  },
  created () {
    this.loading = true
    if (this.statuses) {
      this.statuses.find((element) => {
        ...
        if (element.id === this.id) {
          this.$store.commit('EmployeeStatus/FETCH_OBJECT', element)
          this.loading = false
        }
      })
    }
  ...
  methods: {
    ...mapActions('EmployeeStatus', { findStatus: 'find', deleteStatus: 'delete' }),
    onDelete () {
