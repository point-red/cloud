<template>
  <div>
    ...
    <p-modal :ref="'select-' + id" :id="'select-' + id" title="select similar product">
      <template slot="content">
        ...
        <div class="alert alert-info text-center" v-if="!searchText && options.length == 0 && !isLoading">
          {{ $t('you don\'t have any') | capitalize }} {{ $t('similar product') | capitalize }}
        </div>
      </template>
...
export default {
  ...
  computed: {
    ...mapGetters('pluginPinPointSimilarProduct', ['similarProducts', 'pagination'])
  },
  props: {
    id: {
      type: String,
